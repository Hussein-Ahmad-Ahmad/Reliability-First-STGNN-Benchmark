...
Newey-West HAC standard errors and applies Holm-Bonferroni adjustment
...
OUTPUT_PATH = PROJECT_ROOT / "results" / "task1_point_forecasting" / "dm_metrla_21pairs_mae_aligned.json"
...
def holm_bonferroni(p_values: list) -> list:
    """Apply Holm-Bonferroni adjustment. Returns adjusted p-values."""
    n = len(p_values)
...
def main():
    ...
    if not PRED_DIR.exists():
        ...
        pre_computed = PROJECT_ROOT / "results" / "task1_point_forecasting" / "dm_metrla_21pairs_mae_aligned.json"
    ...
    if raw_pvalues:
        ...
        for i, pair_key in enumerate(pair_order):
            if "p_value" in results[pair_key]:
                results[pair_key]["p_value_holm_adjusted"] = round(adjusted[i], 6)
                results[pair_key]["significant_after_holm"] = adjusted[i] < 0.05
    ...
    sig = sum(1 for k in pair_order if results.get(k, {}).get("significant_after_holm", False))
